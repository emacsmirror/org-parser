...
(defun org-structure/convert-text-tree (text-tree)
  ...
  (when lines
    (let* ((first-line (car lines))
           (first-block (seq-take-while (lambda (line)
                                          (org-structure/descendent? first-line line))
                                        (cdr lines))))
      (cons (cons first-line
                  (org-structure/make-text-tree first-block))
            (org-structure/make-text-tree (seq-drop lines
                                                (+ 1 (length first-block))))))))

(defun org-structure/descendent? (root possible-descendent)
...
For example, a block that starts '* headline' should be in the same block
 at '** nested', but not the same block as '* another headline.'"
  (if (org-structure/headline? root)
      (or (and (org-structure/headline? possible-descendent)
               (< (org-structure/guess-level root)
                  (org-structure/guess-level possible-descendent)))
          (org-structure/plain-list? possible-descendent))
    (and (org-structure/plain-list? possible-descendent)
         (< (org-structure/guess-level root)
            (org-structure/guess-level possible-descendent)))))
